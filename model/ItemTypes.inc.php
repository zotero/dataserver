...
class Zotero_ItemTypes {
	...
	private static $localizedStringsOld = [
	...
	public static function getLocalizedString($typeOrTypeID, $locale='en-US') {
		...
		$strings = \Zotero\Schema::getLocaleStrings($locale)['itemTypes'];
		if (!isset($strings[$itemType])) {
			// TEMP
			if (isset(self::$localizedStringsOld[$itemType])) {
				return self::$localizedStringsOld[$itemType];
			}
			Z_Core::logError("Localized string not found for item type $itemType in $locale");
			return $itemType;
		}
		return $strings[$itemType];
	}
